...
<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 512 512"><path fill="white" d="m0 381.4l237.7-118.9L0 143.6zm237.7-118.9v118.9l237.7-118.9l-237.7-118.9zm237.7-118.9v237.8H512V143.6z"/></svg>
  ...
  const CurrentSong = ({ image, title, artists, id, session }) => {
    const [bibliotecaStatus, setBibliotecaStatus] = useState();
    const { currentMusic } = usePlayerStore((state) => state);
    

      useEffect(() => {
        // Llama a checkSongBibliotecaStatus
        checkSongBibliotecaStatus(id, session);
      }, [currentMusic]);
    

    const checkSongBibliotecaStatus = async (id, session) => {
    try {
      // Serializar el objeto session en formato JSON
      const sessionQuery = encodeURIComponent(JSON.stringify(session));

      // Construir la URL con los parámetros de consulta
      const url = `/api/check-song-biblioteca-status?id=${encodeURIComponent(id)}&session=${sessionQuery}`;

      // Realizar la solicitud GET
      const response = await axios.get(url);
      setBibliotecaStatus(response.data.state)
    ...
    const handleClick = async (operation : string) => {
      ...
      try {
          ...
          await axios.post('/api/set-song-biblioteca', { songId: id, operation: operation, title:title, session:session });
        } catch (error) {
        ...
        {artists && (
          <div className="relative right-1 top-1">
            {bibliotecaStatus ? (
              ...
              >
                <Plus className="w-6 h-6" /> {/* Cambia w-6 h-6 a cualquier tamaño deseado */}
              </button>
            ) : (
              <button
                type="button"
              ...
              >
                <Check className="w-6 h-6" /> {/* Cambia w-6 h-6 a cualquier tamaño deseado */}
              </button>
            )}
          </div>
        )}
...
export const  Player: React.FC<PlayerProps> = ({ state, session }) => {
  ...
  return (
    <div className="flex flex-row justify-between w-full px-1 z-50">
      <div className="min-w-[200px]">
        <CurrentSong {...currentMusic.song}  state={state} session={session}/>
      </div>
