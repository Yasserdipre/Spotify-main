  ...
  const handleButtonClick = async (artist) => {
    try {
      const response = await fetch(
        `/api/get-info-playlist.json?id=${artist.id}`
      );
              ...
              <div key={index} className="text-white text-lg relative">
                <article className="group relative transition-all duration-300 w-44 hover:w-52">
                  <a
                    ...
                    className="playlist-item transition-all duration-300 flex relative p-2 overflow-hidden gap-2 pb-6 rounded-md flex-col"
                  >
                  ...
                  <button
                    ...
                    className="card-play-button rounded-full bg-green-500 p-4 hover:scale-105 transition hover:bg-green-400 absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 group-hover:translate-y-1 transition-all duration-300"
                  >
                    <svg
                      className="w-4 h-4"
                      role="img"
